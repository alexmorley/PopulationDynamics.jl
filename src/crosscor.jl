...
function getstrengthpoint(ICw, spikes::SpikeTimes, lags, cellpairs2use::BitArray)
    ...
    cpind=0
    # for each cell pair
    # for (ind,spiketimes) in enumerate(spikes.spiketimes)
    #    for (ind2,spiketimes2) in enumerate(spikes.spiketimes)
    for (ind,ind2) in zip(findn(cellpairs2use)...)
        cpind += 1       
...
function ptrack_ensemble{T}(processors, spikematrix, ICw::Array{T,2}, ICweights::Array{T,3};
    ...
    try
        ...
        for index in 1:chunksize:length(sliding_spikes)
            ## Chunking
            ...
            for (ind,chunkbit) in enumerate(chunkrange)
